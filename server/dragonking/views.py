...
def buildHouse(request): # 获取创建人openid及房间人数，返回房间id号
    openid = request.GET.get('openid')
    peoplenumber = request.GET.get('number')
    gender_kind = request.GET.get('gender_kind')
    user = User.objects.filter(openid = openid).first()
    user.if_inhouse = True
    user.if_house_owner = True
    new_house = House.objects.create(people_number = peoplenumber,house_owner = user,gender_kind=int(gender_kind))
    new_house.save()
    return JsonResponse({'house_id':new_house.id,'message':'成功创建房间'})
...
def punishCardIndex(request):
    punish_way = request.GET.get('way_for_punishing') # candidate values： truth or challenge
    ...
    return JsonResponse({'punish_card_index':card_index}) # 直接作为数组传输给前端，如果不行则转为HttpResponse尝试
...
def choosePunish(request):
    ...
    new_punishment = PunishChoose.objects.create(user_openid=user_obj,card_id=card_obj) # 生成处罚    对象
    new_punishment.save()
...
def leaveHouse(request): # 用户离开房间
    house_id = request.GET.get('house_id')
    house_obj = House.objects.filter(id=house_id).first()
